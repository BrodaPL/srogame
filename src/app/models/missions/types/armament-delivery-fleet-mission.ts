...
function countSelectedArmamentDeliveryShips(
  ...
  return countArmamentDeliveryShips(
    selection.ships.map((entry) => ({
      type: entry.type,
...
function calculateSelectedArmamentDeliveryHangarUsage(
  ...
  return calculateArmamentDeliveryShipHangarUsage(
    selection.ships.map((entry) => ({
      type: entry.type,
      amount: entry.undamagedAmount + entry.damagedAmount,
    })),
...
export class ArmamentDeliveryFleetMission extends FleetMission {
  ...
  public override getPlannerChecks(context: MissionPlannerContext): MissionCheck[] {
    ...
    if (
      ...
      checks.push({
        ...
        textKey: 'missionPlanner.checks.armamentDeliveryInvalidTarget',
        severity: 'error',
      });
    }
  ...
  private pushArmamentDeliveryChecks(
    ...
    const selectedBombCount = selection.carriedBombs.reduce(
      (total, entry) => total + Math.max(0, Math.floor(entry.amount)),
      0,
    );
    ...
    if (payloadCount <= 0) {
      checks.push({
        ...
        textKey: 'missionPlanner.checks.armamentDeliveryRequiresPayload',
        severity: 'error',
      });
    }
    ...
    if (totalHangarCapacity <= 0) {
      checks.push({
        ...
        textKey: 'missionPlanner.checks.armamentDeliveryRequiresCarrier',
        severity: 'error',
      });
    }
    ...
    if (payloadShipHangarUsage + bombHangarUsage > totalHangarCapacity) {
      checks.push({
        ...
        textKey: 'missionPlanner.checks.armamentDeliveryExceedsHangar',
        severity: 'error',
      });
    }
  }
