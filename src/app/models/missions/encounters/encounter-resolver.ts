...
export class EncounterResolver {
  ...
  public resolvePlanetOrbit(
    ...
    const sortedArrivals = [...arrivals].sort((left, right) =>
      this.compareArrivalPriority(left, right),
    );
    ...
    while (pendingArrivals.length > 0) {
      ...
      const orbitForces = this.createInitialOrbitForces(
        targetPlanet,
        stationaryOccupants,
        coalition,
      );
      ...
      const coalitionHasSurvivors = coalition.some(
        (entry) => ManyShips.totalShipsCount(entry.fleet.ships) > 0,
      );
      const coalitionWon =
        coalitionHasSurvivors &&
        (!hostileDefendersStillPresent ||
          this.totalForceCombatants(hostileDefendersStillPresent) <= 0);
      ...
      for (const member of coalition) {
        ...
        resolvedArrivals.push({
          ...
          outcome: {
            ...
            resolution: survivingShips <= 0 ? 'defeat' : coalitionWon ? 'victory' : 'retreat',
          },
        });
      }
    }

    return resolvedArrivals.sort((left, right) =>
      this.compareArrivalPriority(left.arrival, right.arrival),
    );
  }
  ...
  private createInitialOrbitForces(
    ...
    const coalitionThreatensPlanetOwner =
      coalitionOwnerId !== null &&
    ...
    const coalitionContainsOrbitStayingMission = arrivals.some((entry) =>
      this.isOrbitStayingMission(entry.fleet.missionType),
    );
    ...
    for (const occupant of stationaryOccupants) {
      ...
      if (
        !this.shouldIncludeStationaryOccupant(
          occupant.fleet,
          coalitionOwnerId,
          targetPlanet,
          coalitionThreatensPlanetOwner,
  ...
  private isCoalitionMember(
    ...
    return (
      diplomaticStatus === DiplomaticStatus.SELF || diplomaticStatus === DiplomaticStatus.ALLIED
    );
  }
  ...
  private selectHostileDefenderCoalition(
    ...
    const fallbackSeed = [...hostileForces].sort(
  ...
  private resolveCoalitionBattle(
    ...
    for (const arrival of arrivals) {
      ...
      attackerCargoByFleetId.set(
        arrival.fleet.fleetId,
        new ResourcesPack(
    ...
    for (const defenderEntry of defenderInitialForces) {
      ...
      const allocatedDefences =
        defenderDefenceSurvivorPool.extractAnyDefencesByType(requestedDefences);
  ...
  private toShipAmountRequests(
    ships: ManyShips,
  ): Array<{ type: import('../../enums/ship-type').ShipType; amount: number }> {
  ...
  private calculateBattleDebris(
    ...
    this.addDestroyedShipResources(
      destroyedShipResources,
      ManyShips.toShipInstances(overflowShips),
    );
  ...
  private calculateDefenceDebris(battleResult: SpaceBattleResult): ResourcesPack {
    ...
    this.addDestroyedDefenceResources(
      destroyedDefenceResources,
      battleResult.attacker.destroyedDefences,
    );
    this.addDestroyedDefenceResources(
      destroyedDefenceResources,
      battleResult.defender.destroyedDefences,
    );
