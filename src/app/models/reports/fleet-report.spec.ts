...
import {
  ...
  formatFleetReportCargo,
  formatFleetReportShips,
} from './fleet-report';
import {
...
describe('fleet report ship formatting', () => {
  ...
  it('formats cargo with stable resource labels', () => {
    expect(
      resolveEnglishRuntimeText(
        formatFleetReportCargo({ metal: 120, crystal: 30.8, deuterium: -5 }),
      ),
    ).toBe('Fleet cargo: Metal 120, Crystal 30, Deuterium 0');
  });
  ...
  it('appends stable fleet ships and cargo lines to report body', () => {
    ...
    expect(
      resolveEnglishRuntimeTextBlock(
        appendFleetReportManifest('Spy mission completed successfully.', ships, {
          metal: 1,
          crystal: 2,
          deuterium: 3,
        }),
      ...
      'Spy mission completed successfully.\nFleet ships: Spy Probe x3\nFleet cargo: Metal 1, Crystal 2, Deuterium 3',
    );
  });

  it('handles empty fleets', () => {
    expect(
      resolveEnglishRuntimeTextBlock(appendFleetReportManifest('', ManyShips.empty(), null)),
