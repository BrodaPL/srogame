...
- `server/src/bots-v2/snapshot/build-bot-world-snapshot.ts` now owns the enriched V2 local read model: local incomes/capacities, industry/research/shipyard power, queue ETC estimates, bunker/defense value summaries, installed-ship value summaries plus undamaged/damaged ship splits, building-damage summaries, `avg_industry`, queued-defense/queued-ship state, recent hostile-attack proxy counts, empire-level Strategic Development intel candidates with scan freshness + reported colonization difficulty, active-fleet / active-colonize-fleet counts, strategic-development building levels, and the tech/modifier fields required by the Economic, Defensive, Warfare, and Strategic Development planners, including `anomaliesAndNoise` and `hyperspaceParameters`
