...
- `server/src/bots-v2/execution/bot-executor.ts` owns live execution for accepted `BUILDING`, `RESEARCH`, `SHIPYARD`, and allowlisted `FLEET_MISSION` proposals. It calls shared command helpers, auto-selects own Jump Gate use when legal/auto-approved, splits cargo spending from fuel spending, and logs/traces command failures instead of falling back to V1.
