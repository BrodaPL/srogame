...
- `server/src/bots-v2/jump-gate-operating-cost-policy.ts` owns the V2 Jump Gate operating-cost decision hook. It currently returns a zero-cost quote because Jump Gates have no operating-cost model yet; own-gate auto-selection in the executor already calls it so future costs can be added without spreading cost heuristics through subsystems.
