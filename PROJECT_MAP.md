...
- `server/src/bots-v2/` now owns the shadow-only Bot AI V2 scaffold: feature flags, minimal persisted V2 memory contract, snapshot builder, V2 trace store, stub supervisor/executor, and the Phase 0 `Economic` + `Defensive` subsystems
...
- `server/src/bots-v2/snapshot/build-bot-world-snapshot.ts` now owns the enriched V2 local read model: local incomes/capacities, industry/research/shipyard power, queue ETC estimates, bunker/defense value summaries, `avg_industry`, queued-defense state, recent hostile-attack proxy counts, and the tech/modifier fields required by the Economic and Defensive planners
...
- `server/src/bots-v2/bot-v2-types.ts` now owns the shared V2 goal/result contracts for multiple subsystems instead of Economic-only trace shapes
