...
- `server/src/bots-v2/` now owns the shadow-only Bot AI V2 scaffold: feature flags, minimal persisted V2 memory contract, snapshot builder, V2 trace store, stub supervisor/executor, and the Phase 0 `Economic` + `Defensive` + `Warfare` + `Strategic Development` subsystems
...
- `server/src/bots-v2/subsystems/strategic-development/bot-strategic-development-subsystem.ts` now owns the current two-layer Strategic Development planner: the local phase-1 building/production planner for `INTERSTELLAR_TRADE_PORT` / `JUMP_GATE` / `RESEARCH_LAB` / `SENSOR_PHALANX` plus support-ship readiness stock, and the phase-2 global mission planner that now emits `TRANSPORT`, `ARMAMENT_DELIVERY`, and `SPY` proposals for resource support, repair-drone delivery, and colonization-intel refresh while keeping actual colonization launches deferred
