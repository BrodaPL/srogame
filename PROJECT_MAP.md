...
- `server/src/bots-v2/execution/bot-executor.ts` owns live execution for accepted `BUILDING`, `RESEARCH`, `SHIPYARD`, and allowlisted non-combat `FLEET_MISSION` proposals. It calls shared command helpers, auto-selects own Jump Gate use when legal/auto-approved, and logs/traces command failures instead of falling back to V1.
