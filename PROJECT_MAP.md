...
- `server/src/bots-v2/` now owns the shadow-only Bot AI V2 scaffold: feature flags, minimal persisted V2 memory contract, snapshot builder, V2 trace store, stub supervisor/executor, and the Phase 0 `Economic` + `Defensive` + `Warfare` subsystems
...
- `server/src/bots-v2/bot-v2-types.ts` now owns the shared V2 goal/result contracts for multiple subsystems, including ship-target metadata and Warfare planet results
