...
- `server/src/bots-v2/snapshot/build-bot-world-snapshot.ts` now owns the enriched V2 local read model: local incomes/capacities, industry/research/shipyard power, queue ETC estimates, bunker/defense value summaries, installed-ship value summaries, `avg_industry`, queued-defense/queued-ship state, recent hostile-attack proxy counts, strategic-development building levels, and the tech/modifier fields required by the Economic, Defensive, Warfare, and Strategic Development planners, including `anomaliesAndNoise` and `hyperspaceParameters`
...
- `server/src/bots-v2/subsystems/strategic-development/bot-strategic-development-subsystem.ts` now owns the local Phase 1 Strategic Development planner: per-planet building-side and production-side candidate generation for `INTERSTELLAR_TRADE_PORT` / `JUMP_GATE` / `RESEARCH_LAB` / `SENSOR_PHALANX` plus support-ship readiness stock, strict facility-prerequisite expansion, separate per-queue goal emission, and current analysis-only placeholders for later logistics/colonization/intel work
