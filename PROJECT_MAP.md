...
- `server/src/bots-v2/` now owns the shadow-only Bot AI V2 scaffold: feature flags, persisted V2 memory contract, snapshot builder, V2 trace store, stub supervisor/executor, and the Phase 0 `Economic` + `Defensive` + `Warfare` + `Strategic Development` + `Strategic Military` + `Strategic Diplomatic` + `Weight Manager` + `Critical` subsystems
...
- `server/src/bots-v2/subsystems/critical/bot-critical-subsystem.ts` now owns the emergency-only Critical phase-1 planner: shadow-only detection of energy/storage/industry-chain/logistics/intel deadlocks, fixed family priority, blocker-ledger persistence, earlier-proposal awareness, and capped emergency `BUILDING` / `SHIPYARD` unblock proposals including safe-planet `REPAIR_DRONE` production when damage recovery is otherwise stuck
