...
- `server/src/bots-v2/` now owns the shadow-only Bot AI V2 scaffold: feature flags, minimal persisted V2 memory contract, snapshot builder, V2 trace store, stub supervisor/executor, and the Phase 0 `Economic` + `Defensive` + `Warfare` + `Strategic Development` + `Strategic Military` subsystems
...
- `server/src/bots-v2/snapshot/build-bot-world-snapshot.ts` now owns the enriched V2 local read model: local incomes/capacities, industry/research/shipyard power, queue ETC estimates, bunker/defense value summaries, installed-ship value summaries plus undamaged/damaged ship splits, building-damage summaries, `avg_industry`, queued-defense/queued-ship state, recent hostile-attack proxy counts, empire-level Strategic Development intel candidates with scan freshness + reported colonization difficulty, Strategic Military target snapshots with neutral-vs-not-neutral classification plus report-derived stored-resource/storage/income/bunker/defender observations, active-fleet / active-colonize-fleet counts, strategic-development building levels, and the tech/modifier fields required by the Economic, Defensive, Warfare, Strategic Development, and Strategic Military planners, including `anomaliesAndNoise` and `hyperspaceParameters`
...
- `server/src/bots-v2/subsystems/strategic-military/bot-strategic-military-subsystem.ts` now owns the current global neutral-farm planner: galaxy-wide `SPY` discovery/refresh requests, single-origin `BREAK` attack proposals against scanned neutral defenders, repeatable `PLUNDER` attack proposals against opened farms using cargo plus 1-2 military ships, exact-ship-type `SHIP_NEED` demand proposals when current local fleets cannot execute the next farm step, and the persistent memory-backed farm ledger / regrowth scheduling layer that converts espionage, battle, and plunder reports into remembered neutral-farm state
- `server/src/bots-v2/bot-v2-types.ts` now owns the shared V2 goal/result contracts for multiple subsystems, including ship-target metadata, Warfare planet results, and the separate building-vs-production Strategic Development planet-result shape
