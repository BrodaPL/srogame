...
- `server/src/bots-v2/subsystems/strategic-diplomatic/bot-strategic-diplomatic-subsystem.ts` now owns the current phase-1 global diplomatic-management planner: discovered-faction evaluation, per-faction hostility/stance/strength/confidence scoring, adjacent-only relation-change proposals, incoming/outgoing diplomatic-proposal preference suggestions, retaliation flags, and memory-backed per-faction diplomatic ledgers
