...
- its current implemented subsystems are `Economic`, `Defensive`, `Warfare`, `Strategic Development`, `Strategic Military`, and `Strategic Diplomatic`
...
- the current Strategic Diplomatic shadow planner now has a split phase-1/phase-2 shape: it still tracks only discovered non-neutral players/bots, keeps memory-backed per-faction strength/stance/hostility/confidence ledgers, and emits diplomatic relation-change proposals plus incoming/outgoing proposal-management preferences and retaliation flags, but it now also emits real-player `SPY` mission proposals and up to two per-planet `SPY_PROBE` `SHIP_NEED` requests when diplomatic intel is too old, too shallow, or too sparse across a faction’s known planets; faction spy pressure is weighted by status (`WAR 60%`, `NEUTRAL 25%`, `PEACE 10%`, `ALLIED 5%`) and tries to maximize intel gain per probe spent while explicitly flagging likely enemy espionage superiority
