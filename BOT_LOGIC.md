...
- the current Strategic Development shadow planner now has a split phase-1/phase-2 shape: it still emits separate per-planet building-side and production-side requests for `INTERSTELLAR_TRADE_PORT` / `JUMP_GATE` / `RESEARCH_LAB` / `SENSOR_PHALANX` plus `COLONIZER` / transport / `REPAIR_DRONE` readiness stock, and it now also emits separate global `FLEET_MISSION` proposals for `TRANSPORT`, `ARMAMENT_DELIVERY`, and colonization-intel `SPY` requests using empire-level intel candidates, repair-priority bands, and source/target surplus-shortage matching while actual `COLONIZE` launches remain deferred
