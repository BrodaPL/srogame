...
- Strategic Diplomatic phase-4 update: on top of its existing phase-1/2/3 relation, spy, and attack/support planning, it now also emits `BOMBARD`, `SIEGE`, bombardment-staging `MOVE`, and own/allied `ARMAMENT_DELIVERY` proposals together with direct `BOMB_DEPOT` / `ALLIANCE_DEPOT` / `JUMP_GATE` building pressure and hybrid `PLANETARY_BOMB` production pressure
