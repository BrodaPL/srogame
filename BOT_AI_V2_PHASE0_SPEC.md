...
Budget follow-up TODOs:

- Audit proposal budget attribution after more subsystem work: verify that accepted proposals charge the intended `intentSubsystemId`, `budgetScope`, and `budgetPlanetKey`, especially for cross-subsystem ship needs.
...
- own-planet Jump Gate auto-selection runs through `jump-gate-operating-cost-policy.ts`; Jump Gate travel now has real deuterium cost from `jump-gate-travel-cost.ts`, so auto-selection weighs the paid gate cost against saved turns/fuel instead of blindly using every legal gate
